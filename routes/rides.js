...
const { computeAuthoritativeFareCents } = require("../lib/fareLogic");
...
router.post("/", requireAuth, requireRole("rider"), async (req, res) => {
  const b = req.body || {};
  const { pickup, dest, tier, distanceMiles, durationMin, roundTrip, paymentMethod, scheduledFor, promoCode, useCreditCents } = b;

  if (!pickup || !dest || !["std", "xl", "now"].includes(tier)) {
    return res.status(400).json({ error: "pickup, dest, and tier are required." });
  }
  ...
  // The fare charged is computed here, from the pickup/dest coordinates
  // and a distance/duration that's been sanity-checked against physical
  // reality — never taken directly from whatever the client claims. See
  // lib/fareLogic.js for why: a modified client could otherwise name its
  // own price by sending a fabricated distance for a real trip.
  const { fareCents: fareCentsRounded, distanceMiles: safeDistanceMiles, durationMin: safeDurationMin } = computeAuthoritativeFareCents({
    pickup, dest, tier, roundTrip: !!roundTrip, waitMin: 0,
    claimedDistanceMiles: distanceMiles, claimedDurationMin: durationMin,
  });
...
router.patch("/:id/status", requireAuth, async (req, res) => {
  ...
  res.json({ ride: shapeForRole(updated, req.user.role, req
