...
router.post("/setup-intent", requireAuth, requireRole("rider"), requireStripe, async (req, res) => {
  try{
    ...
    const setupIntent = await stripe.setupIntents.create({
      customer: customerId,
      // "off_session" here because the actual charge later happens when
      // the DRIVER marks the ride complete — the rider isn't necessarily
      // looking at their screen at that exact moment.
      usage: "off_session",
    });
...
// (and store card brand/last4 for display — never the card number itself).
